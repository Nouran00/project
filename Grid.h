...
public:
	...
	void SetallCard7(Player* OwnerPlayer);
	void SetallCard8(Player* OwnerPlayer);
	void SetallCard9(Player* OwnerPlayer);
	void SetallCard10(Player* OwnerPlayer);
	void SetallCard11(Player* OwnerPlayer);
	void SetSemiCard7(Player* MortagePlayer);
	void SetSemiCard8(Player* MortagePlayer);
	void SetSemiCard9(Player* MortagePlayer);
	void SetSemiCard10(Player* MortagePlayer);
	void SetSemiCard11(Player* MortagePlayer);

	void SetOfferMortgage(OfferMortgage* pOfferMortgage);
	...
	void SetClipboardCS(CoinSet* pcoinset);
	CoinSet* GetClipboardCS() const;
	...
	Player* getTherealownerofcard7();
	Player* getTherealownerofcard8();
	Player* getTherealownerofcard9();
	Player* getTherealownerofcard10();
	Player* getTherealownerofcard11();
	...
	Ladder* GetPreviousLadder(const CellPosition& position);  // Gets a Pointer to the first Ladder before the passed "position"
	Snake* GetPreviousSnake(const CellPosition& position);  // Gets a Pointer to the first Snake before the passed "position"
	CoinSet* GetPreviousCoinSet(const CellPosition& position);  // Gets a Pointer to the first CoinSet before the passed "position"
	Card* GetPreviousCard(const CellPosition& position);  // Gets a Pointer to the first Card before the passed "position"
	Cell* GetCell(const CellPosition);
	void SaveAll(ofstream& Output, Type Type)const;
	int Numofladders();
	int Numofsnakes();
	int Numofcards();
	int Numofcoinsset();
